...
SPAWN_RULES[SIM_MODE_NORMAL] = function(b){
    ...
    if(random()<0.00375) b.spawn(false,{x:random(0,WIDTH),y:random(0.5*HEIGHT,0.7*HEIGHT),sType:'l'}); // spawn area for areas like the GoM
...
ENV_DEFS.defaults.LLSteering = {
    ...
    mapFunc: (u,x,y,z)=>{
        ...
        let h = map(cos(map(y,0,HEIGHT,0,PI)),-1.46875,1,1,0);
        // westerlies
...
ENV_DEFS.defaults.SST = {
    ...
    mapFunc: (u,x,y,z)=>{
        ...
        let s = seasonalSine(z);
        let w = map(cos(map(x,0,WIDTH,0,PI)),-0.85,0.85,0.85,0.85);
        let h0 = y/HEIGHT;
        let h1 = (sqrt(h0)+h0)/2;
        let h2 = sqrt(sqrt(h0));
