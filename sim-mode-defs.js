...
SPAWN_RULES[SIM_MODE_NORMAL] = function(b){
    ...
    if(random()<0.0075) b.spawn(false,{x:random(0,WIDTH),y:random(0.5*HEIGHT,0.9*HEIGHT),sType:'l'}); // spawn area for areas like the GoM
...
ENV_DEFS.defaults.SST = {
    ...
    mapFunc: (u,x,y,z)=>{
        ...
        let s = seasonalSine(z);
        let w = map(cos(map(x,0,WIDTH,0,PI)),-0.375,0.375,0.375,0.375);
        let h0 = y/HEIGHT;
        let h1 = (sqrt(h0)+h0)/2;
        let h2 = sqrt(sqrt(h0));
