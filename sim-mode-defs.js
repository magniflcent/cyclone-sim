...
SPAWN_RULES[SIM_MODE_WPAC] = function(b){
    if(random()<0.00375) b.spawn(false,{x:random(0.2*WIDTH,0.7*WIDTH),y:random(0.8*HEIGHT,0.9*HEIGHT),sType:'l'}); //tropics spawn area
    if(random()<0.01-0.002*seasonalSine(b.tick)) b.spawn(true);                 // extratropical cyclones
};
...
ENV_DEFS.defaults.LLSteering = {
    ...
    mapFunc: (u,x,y,z)=>{
        ...
        let trades = constrain(pow(0.50+h+map(ridging,0,1,-0.3,0.3),2)*3,0,3);
