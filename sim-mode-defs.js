...
SPAWN_RULES[SIM_MODE_NORMAL] = function(b){
    if(random()<0.007*sq((seasonalSine(b.tick)+1)/2)+0.002)) b.spawn(false,{x:random(0.025*WIDTH,0.7*WIDTH),y:random(0.6*HEIGHT,0.9*HEIGHT),sType:'l'}); //tropics spawn area
...
ENV_DEFS.defaults.LLSteering = {
    ...
    mapFunc: (u,x,y,z)=>{
        ...
        // westerlies
        let west = constrain(pow(0.5-h+map(u.noise(0),0,1,-0.3,0.3)+map(j,0,HEIGHT,-0.3,0.3),2)*4,0,4);
        // ridging and trades
