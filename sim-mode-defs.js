...
SPAWN_RULES[SIM_MODE_WPAC] = function(b){
	...
	if(random()<0.00025) b.spawn(false,{x:random(0.21*WIDTH,0.239*WIDTH),y:random(0.602*HEIGHT,0.649*HEIGHT),sType:'l'});
	if(random()<0.00025) b.spawn(false,{x:random(0.24*WIDTH,0.269*WIDTH),y:random(0.56*HEIGHT,0.63*HEIGHT),sType:'l'});
	if(random()<0.00055) b.spawn(false,{x:random(0.245*WIDTH,0.269*WIDTH),y:random(0.631*HEIGHT,0.685*HEIGHT),sType:'l'});
	if(random()<0.00025) b.spawn(false,{x:random(0.266*WIDTH,0.301*WIDTH),y:random(0.761*HEIGHT,0.852*HEIGHT),sType:'l'});
	if(random()<0.00025) b.spawn(false,{x:random(0.245*WIDTH,0.265*WIDTH),y:random(0.761*HEIGHT,0.9*HEIGHT),sType:'l'});
	if(random()<0.01-0.002*seasonalSine(b.tick)) b.spawn(true);                 // extratropical cyclones
};
